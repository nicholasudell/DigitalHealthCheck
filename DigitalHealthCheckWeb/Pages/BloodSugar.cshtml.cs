...
{
    public class BloodSugarModel : HealthCheckPageModel
    {
        ...
        public BloodSugarModel(Database database, ICredentialsDecrypter credentialsDecrypter, IPageFlow pageFlow) : base(database, credentialsDecrypter, pageFlow)
        {
        }
        ...
        public async Task OnGetAsync()
        {
            ...
            PreviousPage = (healthCheck?.SexForResults == Sex.Female && healthCheck?.SexAtBirth == Sex.Female) ? "PolycysticOvariesAndGestationalDiabetes" : "Diabetes";
        ...
        public async Task<IActionResult> OnPostAsync(UnsanitisedModel model)
        {
            ...
            if(sanitisedModel.KnowYourHbA1c == HaveYouBeenMeasured.Forgot)
            {
                return RedirectWithId("./FindYourBloodSugar");
            }
        ...
        async Task<SanitisedModel> ValidateAndSanitise(HealthCheck check, UnsanitisedModel model)
        {
            ...
            if (isValid && sanitisedModel.KnowYourHbA1c == HaveYouBeenMeasured.Yes)
            {
